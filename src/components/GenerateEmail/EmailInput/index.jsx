...
const EmailInput = ({ name, label, data, form, allowStyling, currentUser, setIsEmpty }) => {
        ...
        >
          <Select
            ...
            onDeselect={(value) => {
              ...
              if (value === currentUser?.personalDetails?.email) {
                setAddPopUp({ [name]: false });
              }
            }}
          >
            {data && data?.map((item) => <Select.Option key={item?.email}></Select.Option>)}
          </Select>
        </Form.Item>
      </Col>
            ...
            <Button
              ...
              onClick={() => {
                if (
                  !form
                    .getFieldValue(name)
                    ?.find((list) => list === currentUser?.personalDetails?.email)
                ) {
                  form.setFieldsValue({
                    [name]: [
